...
code = re.sub(
    ...
    story.append(Paragraph(f"<b>Client's Signature</b> {client_name}     Date {date.today().strftime('%d.%m.%Y')}", s['sign']))
    story.append(Paragraph(f'Print Name {client_name}', s['sign_sub']))
    story.append(Spacer(1, 10))
    story.append(Paragraph(f"<b>Service Provider Signature</b> RateGuard     Date {date.today().strftime('%d.%m.%Y')}", s['sign']))
...
code = re.sub(
    ...
    r"""
    client_name = calc.get('company_name', 'Client')
    story.append(Paragraph(f"<b>Buyer's Signature</b> {client_name}     Date {date.today().strftime('%d.%m.%Y')}", s['sign']))
    story.append(Paragraph(f'Print Name {client_name}', s['sign_sub']))
    story.append(Spacer(1, 10))
    story.append(Paragraph(f"<b>Supplier's Signature</b> RateGuard     Date {date.today().strftime('%d.%m.%Y')}", s['sign']))
    story.append(Paragraph(f'Print Name RateGuard', s['sign_sub']))""",
    code
)

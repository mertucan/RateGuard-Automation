    ...
    <div className="flex h-full flex-col overflow-hidden bg-bg text-text">
      <header className="flex shrink-0 items-center justify-between border-b border-border bg-surface px-4 py-4 sm:px-8 sm:py-5">
        <div className="min-w-0">
          <h2 className="truncate text-xl font-bold sm:text-2xl">Dashboard Overview</h2>
          <p className="mt-1 hidden text-sm text-text-muted sm:block">Manage renewals and monitor inflation metrics.</p>
        </div>
        <button
        ...
        <div className="space-y-6 sm:space-y-8">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 sm:gap-6">
            ...
            <div className="rounded-xl border border-border bg-surface p-4 sm:p-6">
              ...
              <p className="mt-1 text-2xl font-bold sm:text-3xl">{stats?.pending_approvals ?? '—'}</p>
            ...
            <section className="rounded-xl border border-border bg-surface p-4 sm:p-6 lg:col-span-2">
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 sm:mt-6 sm:gap-4 md:grid-cols-4">
                <div className="rounded-lg border border-border bg-surface-alt p-3 text-center sm:p-4">
                  ...
                  <p className="mt-1 text-xl font-bold sm:text-2xl">{stats?.usd ? stats.usd.toFixed(2) : '—'}</p>
                </div>
                <div className="rounded-lg border border-border bg-surface-alt p-3 text-center sm:p-4">
                  <p className="text-xs font-medium text-text-muted">EUR/TRY</p>
                  <p className="mt-1 text-xl font-bold sm:text-2xl">{stats?.eur ? stats.eur.toFixed(2) : '—'}</p>
                </div>
                <div className="rounded-lg border border-border bg-surface-alt p-3 text-center sm:p-4">
                  <p className="text-xs font-medium text-text-muted">TUFE (YoY)</p>
                  <p className="mt-1 text-xl font-bold text-amber-500 sm:text-2xl">%{tufe.toFixed(1)}</p>
                </div>
                <div className="rounded-lg border border-border bg-surface-alt p-3 text-center sm:p-4">
                  <p className="text-xs font-medium text-text-muted">UFE (YoY)</p>
                  <p className="mt-1 text-xl font-bold text-amber-500 sm:text-2xl">%{ufe.toFixed(1)}</p>
                </div>
              </div>
            </section>
              ...
              <div className="mt-4 space-y-3">
                {tufe > 40 && (
                  <div className="rounded-lg border border-border bg-surface p-4">
                    ...
                    <p className="mt-1 text-xs text-text-muted">TUFE is at %{tufe.toFixed(1)} – contract adjustments may be significant.</p>
                  </div>
                )}
                {stats?.usd > 35 && (
                  <div className="rounded-lg border border-border bg-surface p-4">
                    <p className="text-sm font-semibold">Exchange Rate Warning</p>
                    <p className="mt-1 text-xs text-text-muted">USD/TRY is at {stats.usd.toFixed(2)} – review FX-linked contracts.</p>
                  </div>
                )}
                {tufe <= 40 && (!stats?.usd || stats.usd <= 35) && (
                  <div className="rounded-lg border border-border bg-surface p-4">
                    <p className="text-sm font-semibold">All Clear</p>
                    <p className="mt-1 text-xs text-text-muted">No critical market alerts at this time.</p>
                  </div>
                )}
              </div>
            </section>
          ...
          <section className="overflow-hidden rounded-xl border border-border bg-surface">
            <div className="border-b border-border p-4 sm:p-6">
              ...
              <p className="mt-1 text-sm text-text-muted">Contracts requiring attention based on EVDS data.</p>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-left">
                <thead className="bg-surface-alt">
                  <tr>
                    <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-text-muted sm:px-6 sm:py-4">Client Name</th>
                    <th className="hidden px-4 py-3 text-xs font-semibold uppercase tracking-wider text-text-muted sm:table-cell sm:px-6 sm:py-4">End Date</th>
                    <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-text-muted sm:px-6 sm:py-4">Current Price</th>
                    <th className="hidden px-4 py-3 text-xs font-semibold uppercase tracking-wider text-text-muted md:table-cell sm:px-6 sm:py-4">Calc. New Price</th>
                    <th className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-text-muted sm:px-6 sm:py-4">Adj.</th>
                    <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-text-muted sm:px-6 sm:py-4">Action</th>
                  </tr>
                </thead>
                  ...
                  {rows.map((row) => {
                    ...
                    return (
                      <tr className="transition-colors hover:bg-hover" key={row.id}>
                        ...
                        <td className="hidden whitespace-nowrap px-4 py-3 text-sm sm:table-cell sm:px-6 sm:py-4">{row.end_date || '—'}</td>
                        <td className="whitespace-nowrap px-4 py-3 text-sm sm:px-6 sm:py-4">{formatCurrency(amount)}</td>
                        <td className="hidden whitespace-nowrap px-4 py-3 text-sm font-bold md:table-cell sm:px-6 sm:py-4">{formatCurrency(newPrice)}</td>
                        <td className="whitespace-nowrap px-4 py-3 text-sm font-semibold text-amber-500 sm:px-6 sm:py-4">+{adjPct}%</td>
